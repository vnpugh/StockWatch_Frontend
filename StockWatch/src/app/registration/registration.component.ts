...
export class RegistrationComponent {
  ...
  login(): void {
    let registrationRequest:Register = {email: this.email, password: this.password, firstName: this.name};

    if(!this.email||!this.password||!this.name) {
      this.errorMessage = "The name/email/password cannot be empty";
      return;
    }
    this.httpClient.post("api/auth/users/register", registrationRequest).subscribe({
